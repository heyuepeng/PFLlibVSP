...
def objective(trial):
    ...
    model = Seq2SeqLSTM(input_dim=21, hidden_dim=hidden_dim, decoder_output_dim=1, num_layers=num_layers, future_seq_length=5, dropout=dropout)
    model.to(device)
    optimizer = optim.Adam(model.parameters(), lr=learning_rate)
    criterion = nn.MSELoss()

    train_loader = DataLoader(read_client_data("driving_fg234", 0, True), batch_size=64, shuffle=True, drop_last=True)
    val_loader = DataLoader(read_client_data("driving_fg234", 0, False), batch_size=64, shuffle=False, drop_last=False)
...
with open('best_trial_params_seq2seq_lstm_5.json', 'w') as outfile:
    json.dump(best_trial_params, outfile, indent=4)
print('Best trial:', study.best_trial.params)
print("Saved best trial parameters for Seq2Seq LSTM to best_trial_params_seq2seq_lstm_5.json")
