...
def parse_args():
    ...
    parser.add_argument('-t', "--trials", type=int, default=1, help='Number of independent trials to run (default: 5)')
...
if args.model == "sslstm":
    ...
    if "driving" ==  args.dataset:
        model = DrivingLSTM(input_dim=12, hidden_dim=128, output_dim=5,num_layers=2, bidirectional=False,
                                         dropout=0.1).to(args.device)
    elif "driving_fg24" == args.dataset or "driving_fg34" == args.dataset or "driving_fg34_jr" == args.dataset:
        model = DrivingLSTM(input_dim=18, hidden_dim=128, output_dim=5,num_layers=2, bidirectional=False,
                                         dropout=0.1).to(args.device)
    elif "driving10" == args.dataset:
        model = DrivingLSTM(input_dim=17, hidden_dim=128, output_dim=10, num_layers=2, bidirectional=False,
                                         dropout=0.1).to(args.device)
    elif "driving10_fg24" == args.dataset or "driving10_fg34" == args.dataset or "driving10_fg34_jr" == args.dataset:
        model = DrivingLSTM(input_dim=23, hidden_dim=128, output_dim=10, num_layers=2, bidirectional=False,
                                         dropout=0.1).to(args.device)
