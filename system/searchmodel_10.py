...
from flcore.trainmodel.models import Seq2SeqLSTM
import json
...
def objective(trial):
    ...
    dropout = trial.suggest_float('dropout', 0.0, 0.3)  # Search for dropout rate between 0 and 0.5
    learning_rate = trial.suggest_loguniform('learning_rate', 1e-3, 1e-2)
    # batch_size = trial.suggest_categorical('batch_size', [64, 128])

    model = MultiHead_Seq2SeqLSTM(input_dim=26, hidden_dim=hidden_dim, decoder_output_dim=1, num_layers=num_layers, future_seq_length=10, dropout=dropout)
    model.to(device)
    optimizer = optim.Adam(model.parameters(), lr=learning_rate)
    criterion = nn.MSELoss()

    train_loader = DataLoader(read_client_data("driving10_fg234", 0, True), batch_size=64, shuffle=True, drop_last=True)
    val_loader = DataLoader(read_client_data("driving10_fg234", 0, False), batch_size=64, shuffle=False, drop_last=False)
...
with open('best_trial_params_seq2seq_lstm_10.json', 'w') as outfile:
    json.dump(best_trial_params, outfile, indent=4)
print('Best trial:', study.best_trial.params)
print("Saved best trial parameters for Seq2Seq LSTM to best_trial_params_seq2seq_lstm_10.json")
